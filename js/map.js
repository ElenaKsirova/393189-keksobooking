...
function createAds(adCount) {
  ...
  // можно было воспользоваться IIFE, не вводя generateUserNumStrings,
  // но, мне кажется, так нагляднее
  ...
  // каждый вызов getRandomUserNumString извлекает выбранный элемент
  // из userNumStrings, таким образом гарантируя уникальность
  // выбранного номера пользователя

  var getRandomUserNumString = function () {
    if (!userNumStrings.length) {
      return '';
    }

    var numStringIndex = Math.floor(Math.random() * userNumStrings.length);

    var numString = userNumStrings[numStringIndex];

    userNumStrings.splice(numStringIndex, 1);
  ...
  for (var i = 0; i < adCount; i++) {
    ...
    newAds[i] = {
      'author': {
        'avatar': 'img/avatars/user' + getRandomUserNumString() + '.png'
      },
